...
if ( ! class_exists( 'Advisto_Widget_Area' ) ) {

	class Advisto_Widget_Area {
		...
		function __construct() {
			...
			add_action( 'globaly_render_widget_area', array( $this, 'render' ) );
		}
		...
		public function render( $area_id ) {
			...
			$area_id        = apply_filters( 'globaly_rendering_current_widget_area', $area_id );
			...
			$classes = apply_filters( 'globaly_widget_area_classes', $classes, $area_id );
	...
	function globaly_widget_area() {
		return Advisto_Widget_Area::get_instance();
	}
