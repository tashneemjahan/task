 ...
 */
add_filter( 'template_include', array( 'Advisto_Wrapping', 'wrap' ), 99 );
add_filter( 'globaly_wrap_base', 'globaly_wrap_base_cpts' );
 ...
 */
function globaly_template_path() {
	return Advisto_Wrapping::$main_template;
}
 ...
 */
function globaly_template_base() {
	return Advisto_Wrapping::$base;
}

if ( ! class_exists( 'Advisto_Wrapping' ) ) {
	...
	class Advisto_Wrapping {
		...
		/**
		 * Initialize new instance of Advisto_Wrapping class by setting a variable for the $slug (which defaults to base)
		...
		public function __toString() {
			$this->templates = apply_filters( 'globaly_wrap_' . $this->slug, $this->templates );
		 ...
		 * and $base as static variables in Advisto_Wrapping class.
		 *
		 * @since 1.0.0
 ...
 */
function globaly_wrap_base_cpts( $templates ) {
