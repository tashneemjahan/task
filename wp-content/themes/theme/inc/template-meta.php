 ...
 * @return array
 */
function globaly_share_buttons( $context = 'loop', $args = array(), $config = array() ) {
	...
	if ( ! globaly_is_meta_visible( $meta, $context ) ) {
		return;
	}
	...
	$default_config = apply_filters( 'globaly_default_config_share_buttons', array(
	...
	$share_item_html = apply_filters( 'globaly_share_button_html',
	...
	foreach ( (array) $networks as $id => $network ) :
		...
		$share_buttons .= sprintf(
			...
			esc_html__( 'Share on ', 'globaly' ) . $network['name'],
	...
	printf(
		'<div class="share-btns__list %1$s">%3$s%2$s</div>',
