...
class Advisto_Subscribe_Follow_Widget extends Cherry_Abstract_Widget {
	...
	public function __construct() {
		...
		$this->widget_description = esc_html__( 'Display subscribe form and follow links.', 'globaly' );
		$this->widget_id          = 'globaly_widget_subscribe_follow';
		$this->widget_name        = esc_html__( 'Subscribe and Follow', 'globaly' );
		$this->settings           = array(
			'enable_subscribe' => array(
				...
				'options' => array(
					'enable_subscribe' => esc_html__( 'Enable Subscribe Box', 'globaly' ),
				),
			),
			'subscribe_title' => array(
				'type'  => 'text',
				'value' => esc_html__( 'Subscribe', 'globaly' ),
				'label' => esc_html__( 'Subscribe Title', 'globaly' ),
			),
			'subscribe_message' => array(
				'type'  => 'textarea',
				'label' => esc_html__( 'Subscribe text message', 'globaly' ),
			),
			'subscribe_input' => array(
				'type'  => 'text',
				'value' => esc_html__( 'Enter your email', 'globaly' ),
				'label' => esc_html__( 'Subscribe input placeholder', 'globaly' ),
			),
			'subscribe_submit' => array(
				'type'  => 'text',
				'value' => esc_html__( 'Subscribe', 'globaly' ),
				'label' => esc_html__( 'Subscribe submit label', 'globaly' ),
			),
			'subscribe_success' => array(
				'type'  => 'text',
				'value' => esc_html__( 'You successfully subscribed', 'globaly' ),
				'label' => esc_html__( 'Subscribe success', 'globaly' ),
			),
			'enable_follow' => array(
				'type'   => 'checkbox',
				'value'  => array(
				...
				'options' => array(
					'enable_follow' => esc_html__( 'Enable Follow Box', 'globaly' ),
				),
			),
			'follow_title' => array(
				'type'  => 'text',
				'value' => esc_html__( 'Follow', 'globaly' ),
				'label' => esc_html__( 'Follow Title', 'globaly' ),
			),
			'follow_message' => array(
				'type'  => 'textarea',
				'label' => esc_html__( 'Follow text message', 'globaly' ),
			),
			'enable_background' => array(
				...
				'options' => array(
					'enable_background' => array(
						'label' => esc_html__( 'Enable Custom Background', 'globaly' ),
			...
			'background_image' => array(
				'type'               => 'media',
				'label'              => esc_html__( 'Background Image', 'globaly' ),
				'upload_button_text' => esc_html__( 'Choose Image', 'globaly' ),
				'multi_upload'       => false,
			...
			'invert_text_colorscheme' => array(
				...
				'master'  => 'background_image',
				'options' => array(
					'invert_text_colorscheme' => esc_html__( 'Use "Invert scheme" for text color', 'globaly' ),
				),
			),
			'background_position' => array(
				'type'    => 'select',
				'label'   => esc_html__( 'Background Position', 'globaly' ),
				'value'   => 'center',
				'options' => array(
					'top-left'      => esc_html__( 'Top Left', 'globaly' ),
					'top-center'    => esc_html__( 'Top Center', 'globaly' ),
					'top-right'     => esc_html__( 'Top Right', 'globaly' ),
					'center-left'   => esc_html__( 'Middle Left', 'globaly' ),
					'center'        => esc_html__( 'Middle Center', 'globaly' ),
					'center-right'  => esc_html__( 'Middle Right', 'globaly' ),
					'bottom-left'   => esc_html__( 'Bottom Left', 'globaly' ),
					'bottom-center' => esc_html__( 'Bottom Center', 'globaly' ),
					'bottom-right'  => esc_html__( 'Bottom Right', 'globaly' ),
				),
				'master' => 'background_image',
			),
			'background_repeat' => array(
				...
				'value'   => 'no-repeat',
				'options' => array(
					'repeat'    => esc_html__( 'Repeat', 'globaly' ),
					'repeat-x'  => esc_html__( 'Repeat X', 'globaly' ),
					'repeat-y'  => esc_html__( 'Repeat Y', 'globaly' ),
			...
			'background_size' => array(
				'type'    => 'select',
				'label'   => esc_html__( 'Background Size', 'globaly' ),
				'value'   => 'inherit',
				'options' => array(
					'cover'   => esc_html__( 'Cover', 'globaly' ),
					'contain' => esc_html__( 'Contain', 'globaly' ),
					'auto'    => esc_html__( 'Auto', 'globaly' ),
				),
				'master' => 'background_image',
			),
			'background_color' => array(
				'type'   => 'colorpicker',
				'label'  => esc_html__( 'Background Color', 'globaly' ),
				'master' => 'background_image',
			),
		);

		add_action( 'wp_ajax_globaly_subscribe', array( $this, 'process_subscribe' ) );
		add_action( 'wp_ajax_nopriv_globaly_subscribe', array( $this, 'process_subscribe' ) );

		$this->errors = array(
			'nonce'     => esc_html__( 'Security validation failed', 'globaly' ),
			'mail'      => esc_html__( 'Please, provide valid mail', 'globaly' ),
			'mailchimp' => esc_html__( 'Please, set up MailChimp API key and List ID', 'globaly' ),
			'internal'  => esc_html__( 'Internal error. Please, try again later', 'globaly' ),
		);
	...
	public function widget( $args, $instance ) {
		...
		if ( $subscribe_enabled && $subscribe_template && $api_key && $list_id ) {
			...
			esc_html_e( 'Please set up MailChimp API key and List ID', 'globaly' );
		}
	...
	public function get_social_nav() {
		return globaly_get_social_list( 'widget' );
	}
	...
	public function get_subscribe_submit( $class ) {
		...
		$subscribe_submit = globaly_render_icons( $subscribe_submit );
	...
	public function process_subscribe() {
		...
		if ( ! isset( $_POST['nonce'] ) || ! wp_verify_nonce( $_POST['nonce'], 'globaly_subscribe' ) ) {
...
add_action( 'widgets_init', 'globaly_register_subscribe_follow_widgets' );
function globaly_register_subscribe_follow_widgets() {
	register_widget( 'Advisto_Subscribe_Follow_Widget' );
}
